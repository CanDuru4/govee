...
/// HASS is sending a fan preset mode command
async fn mqtt_fan_preset_mode_command(
    Payload(mode): Payload<String>,
    Params(IdParameter { id }): Params<IdParameter>,
    State(state): State<StateHandle>,
) -> anyhow::Result<()> {
    log::info!("Fan preset mode command for {id}: {mode}");
    let device = state.resolve_device_for_control(&id).await?;

    let work_modes = crate::hass_mqtt::work_mode::ParsedWorkMode::with_device(&device)?;
    let work_mode = work_modes
        .mode_by_name(&mode)
        .ok_or_else(|| anyhow::anyhow!("mode {mode} not found"))?;
